...
2. **Electron desktop app** (`zqclaw-app/`): Install-to-computer version, packaged as DMG/EXE.
...
# Copy to USB drive
cp -R portable/ /Volumes/YOUR_USB/ZqClaw/

# Electron desktop app
cd zqclaw-app && bash setup.sh  # One-click: Node.js + Electron + deps (China mirrors)
...
zqclaw-app/         Electron desktop app (main.js ~400 lines)
...
bootable/           Linux 可启动 U 盘模块（完全独立，不依赖其他模块）
                    ...
                    linux-setup/ — setup-openclaw.sh 安装到 /opt/zqclaw/
                    独立仓库镜像: github.com/dongsheng123132/zqclaw-linux

install/            一键在线安装模块（curl | bash / irm | iex）
                    install.sh (Mac/Linux) + install.ps1 (Windows)
                    7 步流程: 系统检测 → Node.js → OpenClaw → QQ插件 → 技能 → 模型配置 → 启动脚本
                    安装到 ~/.zqclaw/，与 Mac-Install.command 结果相同

```

> **Note**: 官网 (zqclaw.org) 已拆分到独立私有仓库 [zqclaw.org](https://github.com/dongsheng123132/zqclaw.org)，本仓库不再包含 website/ 和 vercel.json。
> **虾航**: AI人导航站 (nav.zqclaw.org) 在独立私有仓库 [xiahang](https://github.com/dongsheng123132/xiahang)。
...
- **Config format**: `{"gateway":{"mode":"local","auth":{"token":"zqclaw"}},"models":{"mode":"merge","providers":{"xxx":{...}}},"agents":{"defaults":{"model":{"primary":"provider/model"}}}}`
...
- `zqclaw-app/node_modules/`, `zqclaw-app/release/`, `zqclaw-app/resources/runtime/`
- `*.dmg`, `*.exe`, `*.blockmap`
...
- External links point to `zqclaw.org` (our site) or `github.com/openclaw/openclaw` (upstream)
...
- Linux x64 (Bootable USB): ✅ `bootable/` 目录 + 独立仓库 [zqclaw-linux](https://github.com/dongsheng123132/zqclaw-linux)

## Bootable Linux Key Details

- **制作环境**: Windows 10/11 + PowerShell (Admin)，4 步脚本
- **U 盘要求**: 32GB+ USB 3.0
- **技术栈**: Ventoy 1.0.99 引导 → Ubuntu 24.04 ISO → casper-rw 持久化 → OpenClaw 安装到 /opt/zqclaw/
- **国内镜像**: ISO 下载走清华/阿里/中科大，Node.js 和 npm 走 npmmirror.com
- **Linux 环境变量**: `OPENCLAW_HOME=/opt/zqclaw/data/.openclaw`
- **bootable/ 完全独立**: 不引用 portable/、zqclaw-app/ 的任何文件，修改互不影响
- **同步**: bootable/ 内容与 zqclaw-linux 仓库保持一致，改一边要记得同步另一边
