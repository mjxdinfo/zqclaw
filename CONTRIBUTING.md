# 贡献指南 / Contributing to ZqClaw

感谢有兴趣参与 ZqClaw 开发！这份文档会告诉你怎么开始、怎么提 issue、怎么提 PR。
...
| `zqclaw-app/` | Electron 桌面 | `npm run dev` / `npm run build:mac-arm64` |
...
2. 使用的形态（portable / install / bootable / zqclaw-app）
3. **完整的错误日志**（贴文字，不要只截图）
4. 你试过哪些步骤
...
git clone https://github.com/<你的用户名>/zqclaw.git
...
- `bootable/` 与独立仓库 `dongsheng123132/zqclaw-linux` 内容保持同步，改一边记得同步另一边

### `install/`

- Mac/Linux 走 `install.sh`，Windows 走 `install.ps1`
- 全部走 npmmirror.com 镜像，不能假设用户能访问 GitHub/npm 官方
- 安装目录固定为 `~/.zqclaw/`（Mac/Linux）或 `%USERPROFILE%\.zqclaw\`（Windows）
