...
chcp 65001 >nul 2>&1
title ZqClaw - Install to Windows

echo.
echo   ========================================
echo     ZqClaw 安装到 Windows
echo     从 U 盘离线安装
echo   ========================================
echo.
...
set "INSTALL_TARGET=%USERPROFILE%\.zqclaw"
...
if "!USE_OPENCLAW!"=="usb" (
    ...
    (echo {"name":"zqclaw-core","version":"1.0.0","private":true,"dependencies":{"openclaw":"latest"}})>"%INSTALL_TARGET%\core\package.json"
...
if not exist "%INSTALL_TARGET%\data\.openclaw\openclaw.json" (
    (echo {"gateway":{"mode":"local","auth":{"token":"zqclaw"}}})>"%INSTALL_TARGET%\data\.openclaw\openclaw.json"
)

REM ---- Copy HTML pages ----
if exist "%UCLAW_DIR%Config.html" copy "%UCLAW_DIR%Config.html" "%INSTALL_TARGET%\" >nul
if exist "%UCLAW_DIR%ZqClaw.html" copy "%UCLAW_DIR%ZqClaw.html" "%INSTALL_TARGET%\" >nul
...
echo.
echo cd /d "%%DIR%%core"
echo start /B "" cmd /c "timeout /t 3 /nobreak ^>nul ^&^& start http://127.0.0.1:!PORT!/#token=zqclaw"
