#!/bin/bash
# ============================================================
# ZqClaw - Install to Mac (从 U 盘安装到电脑)
...
# ---- Copy launch scripts ----
for f in Config.html ZqClaw.html; do
...
for i in $(seq 1 30); do
    ...
    if curl -s -o /dev/null "http://127.0.0.1:$PORT/" 2>/dev/null; then
        open "http://127.0.0.1:$PORT/#token=zqclaw"
        break
    fi
done
...
echo -e "    或终端运行: ${CYAN}bash ~/.zqclaw/start.command${NC}"
echo ""
echo -e "  ${BOLD}首次使用:${NC}"
