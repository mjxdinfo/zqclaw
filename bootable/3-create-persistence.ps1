# ============================================================
#  ZqClaw Bootable USB - Step 3: Create Persistence Image
...
Write-Host "  ZqClaw Bootable USB - Step 3: Persistence" -ForegroundColor Cyan
Write-Host "============================================" -ForegroundColor Cyan
Write-Host ""
...
if ($wslDistro) {
    ...
    $sizeMB = $sizeGB * 1024
    $tmpFile = "/tmp/zqclaw_persistence.dat"
    ...
    $wslNetPath = "\\wsl`$\${wslDistro}\tmp\zqclaw_persistence.dat"
    ...
    if ($dockerWSL) {
        ...
        $dockerOutput = wsl -d $dockerWSL -- sh -c "rm -f $tmpFile && dd if=/dev/zero of=$tmpFile bs=1M count=0 seek=$sizeMB 2>/dev/null && /sbin/mkfs.ext4 -F -L casper-rw $tmpFile >/dev/null 2>&1 && cp $tmpFile /mnt/host/c/zqclaw_persistence_tmp.dat && echo DONE"

        $tmpHostPath = "C:\zqclaw_persistence_tmp.dat"
