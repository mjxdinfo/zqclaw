...
if not exist "%STATE_DIR%\openclaw.json" (
    (echo {"gateway":{"mode":"local","auth":{"token":"zqclaw"}}})>"%STATE_DIR%\openclaw.json"
)

REM Read token from config
set "TOKEN=zqclaw"
if exist "%STATE_DIR%\openclaw.json" (
    for /f "tokens=*" %%t in ('"%NODE_BIN%" -e "try{const c=JSON.parse(require('fs').readFileSync(process.argv[1],'utf8'));console.log((c.gateway&&c.gateway.auth&&c.gateway.auth.token)||'zqclaw')}catch(e){console.log('zqclaw')}" "%STATE_DIR%\openclaw.json"') do set "TOKEN=%%t"
)
...
if "%logchoice%"=="c" (
    set "TS=%date:~0,4%%date:~5,2%%date:~8,2%_%time:~0,2%%time:~3,2%"
    set "TS=!TS: =0!"
    set "EXPORT=%USERPROFILE%\Desktop\zqclaw-logs-!TS!.txt"
...
:uninstall
echo.
echo   === Uninstall ZqClaw ===
echo.
if exist "%USERPROFILE%\.zqclaw" (
    echo   Found installed version: %USERPROFILE%\.zqclaw
    for /f "tokens=*" %%s in ('powershell -command "(Get-ChildItem '%USERPROFILE%\.zqclaw' -Recurse -ErrorAction SilentlyContinue | Measure-Object -Property Length -Sum).Sum / 1MB" 2^>nul') do echo   Size: %%s MB
    echo.
    echo   Type UNINSTALL to delete:
    set /p unconfirm="  > "
    if "!unconfirm!"=="UNINSTALL" (
        rmdir /s /q "%USERPROFILE%\.zqclaw" 2>nul
        echo   Uninstalled!
    ) else (
        echo   Cancelled.
    )
    ...
    echo     Open Settings - Apps - find ZqClaw - Uninstall
)
pause
goto :menu
