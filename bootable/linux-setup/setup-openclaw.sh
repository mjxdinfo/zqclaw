#!/usr/bin/env bash
# ============================================================
#  ZqClaw OpenClaw One-Click Installer for Linux
...
echo "============================================"
echo "  ZqClaw OpenClaw Installer for Linux"
echo "============================================"
echo ""
...
cat > "$INSTALL_DIR/core/package.json" << 'PKGJSON'
{
  ...
  "version": "1.0.0",
  "private": true,
  "description": "ZqClaw OpenClaw Linux runtime"
}
PKGJSON
echo "      Done."
...
if grep -q "boot=casper" /proc/cmdline 2>/dev/null; then
    ...
    # 创建直接启动脚本
    cat > "$REAL_HOME/Desktop/Start-ZqClaw.sh" << 'LAUNCHEREOF'
#!/bin/bash
echo "Starting ZqClaw AI Assistant..."
cd /opt/zqclaw
bash start-openclaw.sh
LAUNCHEREOF
    chmod +x "$REAL_HOME/Desktop/Start-ZqClaw.sh"
    chown "$REAL_USER:$REAL_USER" "$REAL_HOME/Desktop/Start-ZqClaw.sh"
fi

# 仍然创建标准的.desktop文件
cat > "$DESKTOP_FILE" << 'DESKTOPEOF'
[Desktop Entry]
Name=ZqClaw AI Assistant
Comment=OpenClaw AI - Plug and Play
Exec=/opt/zqclaw/start-openclaw.sh
Terminal=true
Type=Application
Categories=Utility;
...
echo ""
echo "  To start:  Double-click 'ZqClaw AI Assistant' on desktop"
