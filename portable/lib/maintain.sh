#!/bin/bash
# ============================================================
# ZqClaw Maintenance Functions (shared by Mac menus)
...
# Detect install mode: installed (~/.zqclaw), usb (/Volumes or /media), portable (other)
detect_install_mode() {
    if echo "$UCLAW_DIR" | grep -q "/.zqclaw"; then
        echo "installed"
...
do_logs() {
    ...
    case $LOG_CHOICE in
        ...
        b)
            ...
            local EXPORT_FILE="$EXPORT_DIR/zqclaw-logs-$TS.txt"
...
do_uninstall() {
    ...
    case $MODE in
        installed)
            echo "  检测到: 已安装版本 (~/.zqclaw/)"
            echo ""
            echo "  将删除以下目录:"
            echo -e "    ${YELLOW}$HOME/.zqclaw/${NC}"
            if [ -d "$HOME/.zqclaw" ]; then
                echo "    大小: $(du -sh "$HOME/.zqclaw" 2>/dev/null | cut -f1)"
            fi
            echo ""
        ...
        usb)
            ...
            echo ""
            echo "  便携版不需要卸载 —— 只需:"
            echo "    1. 关闭所有 ZqClaw 窗口"
            echo "    2. 安全弹出 U 盘"
            echo "    3. 如需清理电脑上的数据，删除 ~/.zqclaw/ (如果存在)"
            echo ""
            if [ -d "$HOME/.zqclaw" ]; then
                echo -e "  ${YELLOW}发现本机数据: ~/.zqclaw/${NC}"
                echo "  大小: $(du -sh "$HOME/.zqclaw" 2>/dev/null | cut -f1)"
        ...
        portable)
            ...
            echo ""
            echo "  Electron 桌面版卸载:"
            echo "    macOS: 将 ZqClaw.app 从「应用程序」拖到废纸篓"
            ;;
    esac
}
