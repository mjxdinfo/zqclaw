...
const server = http.createServer((req, res) => {
  ...
  // API: Re-run bootstrap to inject the zqclaw-cloud provider
  ...
  // API: Remove zqclaw-cloud provider so the next bind regenerates it
  if (req.url === '/api/xiapan/unbind' && req.method === 'POST') {
    try {
      if (fs.existsSync(CONFIG_PATH)) {
        const config = JSON.parse(fs.readFileSync(CONFIG_PATH, 'utf8'));
        if (config.models && config.models.providers && config.models.providers['zqclaw-cloud']) {
          delete config.models.providers['zqclaw-cloud'];
          fs.writeFileSync(CONFIG_PATH, JSON.stringify(config, null, 2));
        }
      }
      res.writeHead(200, { 'Content-Type': 'application/json' });
