...
for i in $(seq 1 30); do
    ...
    if curl -s -o /dev/null "http://127.0.0.1:$PORT/" 2>/dev/null; then
        open "http://127.0.0.1:$PORT/#token=zqclaw" 2>/dev/null || true
        break
    fi
done

echo ""
echo "  🦞 ZqClaw 正在运行: http://127.0.0.1:$PORT/#token=zqclaw"
echo "  按 Ctrl+C 停止"
echo ""
...
xdg-open "http://127.0.0.1:$PORT/#token=zqclaw" 2>/dev/null || true
wait $PID
STARTEOF
...
echo -e "  ${BOLD}启动方式:${NC}"
if [ "$OS" = "Darwin" ]; then
    echo -e "    双击 ${CYAN}~/.zqclaw/start.command${NC}"
    echo -e "    或终端: ${CYAN}bash ~/.zqclaw/start.command${NC}"
else
    echo -e "    终端: ${CYAN}bash ~/.zqclaw/start.sh${NC}"
fi
echo ""
echo -e "  ${BOLD}打开后:${NC}"
echo -e "    浏览器自动打开 → 开始和 AI 对话"
echo ""
echo -e "  ${DIM}如需重新配置模型，编辑 ~/.zqclaw/data/.openclaw/openclaw.json${NC}"
echo -e "  ${DIM}卸载: rm -rf ~/.zqclaw${NC}"
echo ""
