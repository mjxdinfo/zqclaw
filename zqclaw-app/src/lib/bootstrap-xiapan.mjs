// Bootstrap: ensure data/.openclaw/openclaw.json contains the zqclaw-cloud provider
...
const PROVIDER_ID = 'zqclaw-cloud';
...
const DEFAULT_PROVIDER_TEMPLATE = {
  baseUrl: 'https://api.zqclaw.org/v1',
...
// Set agents.defaults.model.primary to zqclaw-cloud only when the user has not
...
export async function bootstrapXiapan({ configPath, appRoot, log = console } = {}) {
  ...
  const config = readJsonSafe(configPath) || { gateway: { mode: 'local', auth: { token: 'zqclaw' } } };
  ...
  if (existing && typeof existing === 'object') {
    if (existing.apiKey && existing.apiKey !== apiKey) {
      log.info?.(
        `[bootstrap-xiapan] zqclaw-cloud apiKey already configured (different fingerprint). `
        + `Current source=${fingerprintInfo.source}. Use Config UI to rebind if needed.`,
      );
      // Still ensure agents.defaults exists so the runtime knows to use zqclaw-cloud
  ...
  writeJson(configPath, config);
  log.info?.(
    `[bootstrap-xiapan] Wrote zqclaw-cloud provider (source=${fingerprintInfo.source}, key=${apiKey.slice(0, 12)}…)`,
  );
  return {
    ok: true,
